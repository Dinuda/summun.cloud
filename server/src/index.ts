...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.SUMMUN_SECRETS_PROVIDER === undefined) {
    process.env.SUMMUN_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.SUMMUN_SECRETS_STRICT_MODE === undefined) {
    process.env.SUMMUN_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.SUMMUN_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.SUMMUN_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.SUMMUN_MIGRATION_PROMPT === "never") return false;
    if (process.env.SUMMUN_MIGRATION_AUTO_APPLY === "true") return true;
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.SUMMUN_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.SUMMUN_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or SUMMUN_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.SUMMUN_LISTEN_HOST = runtimeListenHost;
  process.env.SUMMUN_LISTEN_PORT = String(listenPort);
  process.env.SUMMUN_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.SUMMUN_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.SUMMUN_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
