...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.SUMMUN_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.SUMMUN_AGENT_JWT_SECRET === "string" ? parsed.SUMMUN_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm summun onboard`)",
  };
}
