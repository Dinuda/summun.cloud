...
function renderPaperclipEnvNote(env: Record<string, string>): string {
  const paperclipKeys = Object.keys(env)
    .filter((key) => key.startsWith("SUMMUN_"))
    .sort();
  if (paperclipKeys.length === 0) return "";
  return [
    "Paperclip runtime note:",
    `The following SUMMUN_* environment variables are available in this run: ${paperclipKeys.join(", ")}`,
...
export async function ensureCursorSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[summun] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[summun] Failed to read Paperclip skills from ${skillsDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[summun] Injected Cursor skill "${entry.name}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[summun] Failed to inject Cursor skill "${entry.name}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  if (wakeTaskId) {
    env.SUMMUN_TASK_ID = wakeTaskId;
  }
  if (wakeReason) {
    env.SUMMUN_WAKE_REASON = wakeReason;
  }
  if (wakeCommentId) {
    env.SUMMUN_WAKE_COMMENT_ID = wakeCommentId;
  }
  if (approvalId) {
    env.SUMMUN_APPROVAL_ID = approvalId;
  }
  if (approvalStatus) {
    env.SUMMUN_APPROVAL_STATUS = approvalStatus;
  }
  if (linkedIssueIds.length > 0) {
    env.SUMMUN_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  }
  if (effectiveWorkspaceCwd) {
    env.SUMMUN_WORKSPACE_CWD = effectiveWorkspaceCwd;
  }
  if (workspaceSource) {
    env.SUMMUN_WORKSPACE_SOURCE = workspaceSource;
  }
  if (workspaceId) {
    env.SUMMUN_WORKSPACE_ID = workspaceId;
  }
  if (workspaceRepoUrl) {
    env.SUMMUN_WORKSPACE_REPO_URL = workspaceRepoUrl;
  }
  if (workspaceRepoRef) {
    env.SUMMUN_WORKSPACE_REPO_REF = workspaceRepoRef;
  }
  if (workspaceHints.length > 0) {
    env.SUMMUN_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  }
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[summun] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[summun] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
