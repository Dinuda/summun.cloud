...
function buildAgentEnvExports(input: {
  ...
  return [
    `export SUMMUN_API_URL='${escaped(input.apiBase)}'`,
    `export SUMMUN_COMPANY_ID='${escaped(input.companyId)}'`,
    `export SUMMUN_AGENT_ID='${escaped(input.agentId)}'`,
    `export SUMMUN_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
