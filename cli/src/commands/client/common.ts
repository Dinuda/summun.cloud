...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", "Paperclip data directory root (isolates state from ~/.summun)")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.SUMMUN_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.SUMMUN_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.SUMMUN_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set SUMMUN_COMPANY_ID, or set context profile companyId via `paperclipai context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.SUMMUN_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.SUMMUN_SERVER_PORT || "");
