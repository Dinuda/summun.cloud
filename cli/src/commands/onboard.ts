...
const ONBOARD_ENV_KEYS = [
  "SUMMUN_PUBLIC_URL",
  "DATABASE_URL",
  "SUMMUN_DB_BACKUP_ENABLED",
  "SUMMUN_DB_BACKUP_INTERVAL_MINUTES",
  "SUMMUN_DB_BACKUP_RETENTION_DAYS",
  "SUMMUN_DB_BACKUP_DIR",
  "SUMMUN_DEPLOYMENT_MODE",
  "SUMMUN_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "SUMMUN_ALLOWED_HOSTNAMES",
  "SUMMUN_AUTH_BASE_URL_MODE",
  "SUMMUN_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "SUMMUN_STORAGE_PROVIDER",
  "SUMMUN_STORAGE_LOCAL_DIR",
  "SUMMUN_STORAGE_S3_BUCKET",
  "SUMMUN_STORAGE_S3_REGION",
  "SUMMUN_STORAGE_S3_ENDPOINT",
  "SUMMUN_STORAGE_S3_PREFIX",
  "SUMMUN_STORAGE_S3_FORCE_PATH_STYLE",
  "SUMMUN_SECRETS_PROVIDER",
  "SUMMUN_SECRETS_STRICT_MODE",
  "SUMMUN_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.SUMMUN_PUBLIC_URL?.trim() ||
    process.env.SUMMUN_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.SUMMUN_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.SUMMUN_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.SUMMUN_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.SUMMUN_ALLOWED_HOSTNAMES
    ? process.env.SUMMUN_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.SUMMUN_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.SUMMUN_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.SUMMUN_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.SUMMUN_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.SUMMUN_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.SUMMUN_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.SUMMUN_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.SUMMUN_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.SUMMUN_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.SUMMUN_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.SUMMUN_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.SUMMUN_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.SUMMUN_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.SUMMUN_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.SUMMUN_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "SUMMUN_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" summun onboard ")));
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `summun doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("SUMMUN_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.SUMMUN_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("SUMMUN_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("SUMMUN_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }
  ...
  p.note(
    [
      ...
      "Agent auth: SUMMUN_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("summun run")}`,
      `Reconfigure later: ${pc.cyan("summun configure")}`,
      `Diagnose setup: ${pc.cyan("summun doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.SUMMUN_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("summun run")}`,
        `Then: ${pc.cyan("summun auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
