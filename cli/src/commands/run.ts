...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  process.env.SUMMUN_INSTANCE_ID = instanceId;
  ...
  p.intro(pc.bgCyan(pc.black(" summun run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("summun onboard")} once, then retry ${pc.cyan("summun run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Summun server...");
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  try {
    ...
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Summun server entrypoint.\n` +
          `Tried: ${devEntry}, @paperclipai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Summun server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Summun server entrypoint did not export startServer(): ${label}`);
  }
