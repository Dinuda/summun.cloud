...
describe("applyDataDirOverride", () => {
  beforeEach(() => {
    ...
    delete process.env.SUMMUN_CONFIG;
    delete process.env.SUMMUN_CONTEXT;
  ...
  it("sets SUMMUN_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.SUMMUN_HOME).toBe(expectedHome);
    expect(process.env.SUMMUN_CONFIG).toBe(
      path.resolve(expectedHome, "instances", "default", "config.json"),
    );
    expect(process.env.SUMMUN_CONTEXT).toBe(path.resolve(expectedHome, "context.json"));
    expect(process.env.SUMMUN_INSTANCE_ID).toBe("default");
  });
  ...
  it("uses the provided instance id when deriving default config path", () => {
    ...
    expect(process.env.SUMMUN_INSTANCE_ID).toBe("dev_1");
    expect(process.env.SUMMUN_CONFIG).toBe(
  ...
  it("does not override explicit config/context settings", () => {
    process.env.SUMMUN_CONFIG = "/env/config.json";
    process.env.SUMMUN_CONTEXT = "/env/context.json";
    ...
    expect(process.env.SUMMUN_CONFIG).toBe("/env/config.json");
    expect(process.env.SUMMUN_CONTEXT).toBe("/env/context.json");
  });
  ...
  it("only applies defaults for options supported by the command", () => {
    ...
    expect(process.env.SUMMUN_HOME).toBe(path.resolve("/tmp/paperclip-alt"));
    expect(process.env.SUMMUN_CONFIG).toBeUndefined();
    expect(process.env.SUMMUN_CONTEXT).toBeUndefined();
  });
});
