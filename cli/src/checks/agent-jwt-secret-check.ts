...
export function agentJwtSecretCheck(configPath?: string): CheckResult {
  if (readAgentJwtSecretFromEnv(configPath)) {
    return {
      ...
      status: "pass",
      message: "SUMMUN_AGENT_JWT_SECRET is set in environment",
    };
  }
  ...
  if (fileSecret) {
    return {
      name: "Agent JWT secret",
      status: "warn",
      message: `SUMMUN_AGENT_JWT_SECRET is present in ${envPath} but not loaded into environment`,
  ...
  return {
    name: "Agent JWT secret",
    status: "fail",
    message: `SUMMUN_AGENT_JWT_SECRET missing from environment and ${envPath}`,
    canRepair: true,
    repair: () => {
      ensureAgentJwtSecret(configPath);
    },
    repairHint: `Run with --repair to create ${envPath} containing SUMMUN_AGENT_JWT_SECRET`,
  };
}
